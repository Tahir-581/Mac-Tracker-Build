...
def resource_path(relative_path: str) -> str:
    ...
    to the temporary folder where PyInstaller extracts files.
    In development mode, use __file__ to get the script directory.
    
    Args:
        relative_path: Path relative to the script/executable location
...
BACKEND_API_BASE = "http://101.53.247.91:8090"  # Default backend API base URL
...
# Log file in user's home directory (not in PyInstaller temp dir)
# This ensures logs persist and are accessible
...
def get_system_info_cmd(cmd: str, platform_specific: bool = False) -> str:
    """
    Cross-platform helper to run system commands and return the result.
    
    Args:
        cmd: Command to run (Windows-specific if platform_specific=True)
        platform_specific: If True, only run on Windows
        
    Returns:
        Command output or "unknown" on failure
    """
    if platform_specific and platform.system() != 'Windows':
        return "unknown"
...
def collect_system_metadata(writer_name: str, title: str) -> dict:
    ...
    # Enhanced Hardware Specs - Cross-platform
    cpu_name = "unknown"
    gpu_name = "unknown"
    ...
    if system == 'Windows':
        ...
        cpu_name = get_system_info_cmd("wmic cpu get name", platform_specific=True)
        gpu_name = get_system_info_cmd("wmic path win32_VideoController get name", platform_specific=True)
    elif system == 'Darwin':  # macOS
        # macOS-specific commands using sysctl
        try:
...
def main():
    ...
    if platform.system() == 'Darwin':
        # On macOS, mss requires screen recording permissions
        # This is a warning, not a fatal error - the app will fail gracefully if permissions aren't granted
        logger.info("macOS detected - ensure Screen Recording permissions are granted in System Preferences")
